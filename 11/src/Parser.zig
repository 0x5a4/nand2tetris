...
fn parseFunctionBody(self: *Self) Error!NodeIndex {
    ...
    var child_count: u32 = 1; // starts at one because of statements
...
fn parseClassDecl(self: *Self) Error!NodeIndex {
    ...
    while (true) : (child_count += 1) {
        _ = try self.tasteToken(&.{ .keyword_static, .keyword_field }) orelse
            break;

        _ = try self.parseClassVarDecl();
    }

    while (true) : (child_count += 1) {
        _ = try self.tasteToken(&.{
            .keyword_constructor,
            .keyword_method,
            .keyword_function,
        }) orelse
